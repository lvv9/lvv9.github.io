...
```
最终可以得出取得NioEventLoopGroup中的一个NioEventLoop执行注册，在NioEventLoopGroup的构造过程中，会执行newChild得到NioEventLoop（且无参构造器只会有一个）
```text
...
```
fireChannelRead时，ServerBootstrapAcceptor开始工作，执行
```text
...
```
至此，服务端的主要工作完成，剩下的就是NioSocketChannel的工作。