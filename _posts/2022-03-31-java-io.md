...
信号处理程序可以在自己的"线程"上从内核复制数据，这样的话和异步就比较IO像了，但因为并发安全的问题一般不建议这么做。
5. 异步IO
...
- 预处理 替换源码中的一些字符串
- 编译 将预处理后的源码翻译成成汇编
- 汇编 将汇编的代码翻译成一个可定位目标文件（二进制）
- 链接 将多个可定位目标文件合并成可执行目标文件
- 加载 不同的启动程序，将可执行的文件的部分结构，复制到此进程独占的空间，然后将控制权转移到此进程的代码开头

Java类似，只不过在操作系统提供抽象的基础上，更进了一步，抽象出了JVM一层，Java应用（包括Java、Scala）只需要将源文件编译（广义的编译）成字节码即可。

从以上过程来看，如果要同时运行多个应用，那么
- 直接访问 编译时为不同的应用分配不同的地址段，所有应用代码访问空间时访问同一空间的不同部分，应用可以感知自己所处位置与其它应用不同
- 段基址 加载时为不同的应用分配不同的地址段，所有应用代码访问空间时访问同一空间的不同部分，应用可以感知自己所处位置与其它应用不同
- 虚拟地址 加载时为不同的应用分配不同的空间，运行时透明地为不同应用切换不同空间，应用不可以感知自己所处位置与其它应用不同

参考 [彻底搞懂虚拟内存，虚拟地址，虚拟地址空间](https://www.toutiao.com/article/6952760982372385317/)
