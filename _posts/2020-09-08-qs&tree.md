...
这样，做多少轮比较，时间复杂度就是多少乘以n。因快排将队列分成了左边和右边，也即是说可以将这个过程想像成一颗二叉树。在最好的情况下，就是树的深度最浅的情况，需要log n轮。在最坏的情况下，树退化成列表，需要n轮。所以最好和最坏的复杂度分别为n*log n和n^2。在平均情况下，这还是一棵二叉树，深度差不多也是log n（乘以一个系数）。
...
## 跳表
跳表与B+树很像，在原始链表的基础上一级一级地增加索引实现。增删查的复杂度都为log n。
